...
ssh -i "$KEY" \
  ...
  "$USER@$HOST" \
  'cd ~/quoreb2b-crm && git fetch origin main && git clean -fd && git reset --hard origin/main && bash deploy/ec2/ensure-production-env.sh && bash deploy/ec2/deploy-backend.sh'
