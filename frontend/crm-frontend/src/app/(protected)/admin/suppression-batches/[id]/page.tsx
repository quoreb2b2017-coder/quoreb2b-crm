...
export default function AdminSuppressionBatchViewPage() {
  ...
  useEffect(() => {
    ...
    batchesService
      ...
      .catch(() => setError('Could not load suppression campaign.'))
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <div className="text-center">
          ...
          <button
            type="button"
            onClick={() => router.push('/admin/suppression-batches')}
            className="mt-4 text-sm text-slate-500 underline"
          >
            Back to suppression campaigns
          </button>
        </div>
      </div>
    );
  ...
  return (
    <BatchExcelView
      batchId={id}
      editable
      name={batch?.name ?? 'Suppression campaign'}
      ...
      onClose={() => router.push('/admin/suppression-batches')}
    />
  );
}
