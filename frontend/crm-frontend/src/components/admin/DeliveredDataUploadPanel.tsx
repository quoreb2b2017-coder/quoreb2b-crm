...
export function DeliveredDataUploadPanel() {
    ...
    async (payload: SpreadsheetData, mode: 'append' | 'replace' = 'append') => {
      ...
      try {
        ...
        if (mode === 'append' && record.addedRows != null) {
          ...
          toast.success('Added to delivered database', `+${record.addedRows} contacts · ${record.rowCount} total${skipped}`);
          ...
          toast.success('Saved to database', `${record.rowCount} contacts in delivered data`);
          window.dispatchEvent(new CustomEvent('delivered-data-updated'));
        }
      } catch (e) {
  ...
  const handleSaveBatch = async () => {
    ...
    try {
      ...
      await loadCoverage();
      if (result.batch) {
        toast.success('Delivered batch created!', `"${result.batch.name}" — ${result.batch.rowCount} contacts`);
              ...
              <div className="space-y-4 px-6 py-5">
                ...
                <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                  <p className="font-medium">
                    {duplicateModal.duplicateCount} contact(s) were already in a delivered batch
