...
import { DeliveredDataClearConfirmModal } from '@/components/delivered-data/DeliveredDataClearConfirmModal';
...
export function DeliveredDataUploadPanel() {
    ...
    async (mode: 'append' | 'replace', record: Awaited<ReturnType<typeof deliveredDataService.save>>, fileName: string) => {
      try {
        await activityLogsService.track({
          ...
          resource: 'delivered-data',
          path: '/admin/delivered-file',
    ...
    async (payload: SpreadsheetData, mode: 'append' | 'replace' = 'append') => {
      ...
      try {
        ...
        if (mode === 'append' && record.addedRows != null) {
          ...
          toast.success('Added to delivered database', `+${record.addedRows} rows · ${record.rowCount} total${skipped}`);
          ...
          toast.success('Saved to database', `${record.rowCount} rows in delivered data`);
          window.dispatchEvent(new CustomEvent('delivered-data-updated'));
        }
      } catch (e) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('delivered-data-updated', refresh);
    window.addEventListener('delivered-batch-created', refresh);
    return () => {
      window.removeEventListener('delivered-data-updated', refresh);
      window.removeEventListener('delivered-batch-created', refresh);
    };
  }, [loadCoverage]);
  ...
  const handleSampleTemplate = async () => {
    try {
      await downloadSpreadsheetXlsx(getSampleMasterData(), 'delivered-data-template.xlsx');
  ...
  const confirmClearData = async () => {
    ...
    try {
      ...
      toast.success(
        'Delivered data removed',
        `${n} delivered batch${n === 1 ? '' : 'es'} deleted from database`,
      );
  ...
  const openBatchModal = useCallback(
    (payload: { rows: string[][]; headers: string[]; sourceRowIndices: number[] }) => {
      if (!payload.sourceRowIndices.length) {
        toast.error('No rows selected', 'Apply filters or pick rows that are not already in a delivered batch');
        return;
      }
  ...
  const handleSaveBatch = async () => {
    ...
    try {
      ...
      await loadCoverage();
      if (result.batch) {
        toast.success('Delivered batch created!', `"${result.batch.name}" — ${result.batch.rowCount} rows`);
      ...
      if (result.duplicateCount > 0) {
        setDuplicateModal({
          fileName: data?.fileName ?? 'delivered-batch',
        ...
        router.push('/admin/delivered-batches');
      }
    } catch (e) {
      toast.error('Delivered batch creation failed', extractApiError(e, 'Could not create delivered batch'));
    ...
    <div className="flex h-full min-h-0 w-full flex-col">
      ...
      <div className="flex flex-col gap-2 border-b border-[#d4d4d4] bg-[#f3f3f3] px-3 py-2 text-sm sm:flex-row sm:flex-wrap sm:items-center sm:gap-x-4 sm:gap-y-2 sm:px-4">
        <div className="flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1">
          <span className="font-semibold text-slate-800">Delivered File Upload</span>
          {data ? (
            <>
      ...
      {loadingDb && !data ? (
        <div className="flex-1 flex items-center justify-center bg-white min-h-[400px] text-sm text-slate-500">
          <Loader2 className="h-5 w-5 animate-spin mr-2" />Loading delivered data from database…
        </div>
      ) : !data ? (
        <div
        ...
        <div className="flex-1 min-h-0 p-0 bg-[#e6e6e6]">
          <ExcelPreviewGrid
            data={data}
            dataResetKey={savedAt ?? 'delivered-empty'}
            editable
        ...
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-end justify-center p-0 sm:items-center sm:p-4 pointer-events-none">
            <div
              role="dialog"
              aria-modal="true"
              aria-labelledby="create-delivered-batch-title"
            ...
            >
              ...
              <div className="flex shrink-0 items-start justify-between gap-3 border-b border-slate-100 px-4 py-4 sm:px-6">
                <div className="min-w-0">
                  <p id="create-delivered-batch-title" className="font-semibold text-slate-900">
                    Create Delivered Batch
                  </p>
              ...
              <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain px-4 py-4 sm:px-6 sm:py-5">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="What is this delivered batch for?"
                    rows={2}
                ...
                <div className="space-y-3 rounded-xl border border-slate-200 bg-slate-50 p-3 sm:p-4">
                  ...
                  <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2.5">
                    <p className="text-xs leading-relaxed text-emerald-800">
                      Delivered file stays unchanged. Rows already in a delivered batch show as{' '}
              ...
              <div className="space-y-4 px-6 py-5">
                ...
                <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                  <p className="font-medium">
                    {duplicateModal.duplicateCount} row(s) were already in a delivered batch
      ...
      <DeliveredDataClearConfirmModal
        open={clearModalOpen}
